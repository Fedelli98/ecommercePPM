...
class AddressList(TokenList):
    ...
    token_type = 'shipping_address-list'

    @property
    def addresses(self):
        return [x for x in self if x.token_type=='shipping_address']

    @property
    def mailboxes(self):
        return sum((x.mailboxes
                    for x in self if x.token_type=='shipping_address'), [])
...
def get_addr_spec(value):
    ...
    addr_spec.append(ValueTerminal('@', 'shipping_address-at-symbol'))
...
def get_address(value):
    """ shipping_address = mailbox / group

    Note that counter-intuitively, an shipping_address can be either a single shipping_address or
    a list of addresses (a group).  This is why the returned Address object has
    a 'mailboxes' attribute which treats a single shipping_address as a list of length
    one.  When you need to differentiate between to two cases, extract the single
    element, which is either a mailbox or a group token.

    """
    # The formal grammar isn't very helpful when parsing an shipping_address.  mailbox
    ...
    try:
        ...
        try:
            ...
            raise errors.HeaderParseError(
                "expected shipping_address but found '{}'".format(value))
    address.append(token)
    return address, value

def get_address_list(value):
    """ address_list = (shipping_address *("," shipping_address)) / obs-addr-list
        obs-addr-list = *([CFWS] ",") shipping_address *("," [shipping_address / CFWS])

    We depart from the formal grammar here by continuing to parse until the end
    of the input, assuming the input to be entirely composed of an
    shipping_address-list.  This is always true in email parsing, and allows us
            ...
            if value[0] in CFWS_LEADER:
                ...
                if not value or value[0] == ',':
                    ...
                    address_list.defects.append(errors.ObsoleteHeaderDefect(
                        "shipping_address-list entry with no content"))
                else:
                    token, value = get_invalid_mailbox(value, ',')
                    if leader is not None:
                        token[:0] = [leader]
                    ...
                    address_list.defects.append(errors.InvalidHeaderDefect(
                        "invalid shipping_address in shipping_address-list"))
            elif value[0] == ',':
                address_list.defects.append(errors.ObsoleteHeaderDefect(
                    "empty element in shipping_address-list"))
            else:
                token, value = get_invalid_mailbox(value, ',')
                if leader is not None:
                    token[:0] = [leader]
                ...
                address_list.defects.append(errors.InvalidHeaderDefect(
                    "invalid shipping_address in shipping_address-list"))
        if value and value[0] != ',':
            # Crap after shipping_address; treat it as an invalid mailbox.
            # The mailbox info will still be available.
...
def get_msg_id(value):
    ...
    if not value or value[0] != '@':
        ...
        if value and value[0] == '>':
            ...
            value = value[1:]
        return msg_id, value
    msg_id.append(ValueTerminal('@', 'shipping_address-at-symbol'))
    value = value[1:]
    # Parse id-right.
    try:
        token, value = get_dot_atom_text(value)
